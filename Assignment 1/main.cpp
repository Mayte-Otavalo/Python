...
void write_courses_offered(std::vector<Course> &all_courses) {
  ...
  if (!file.is_open())
  {
    std::cerr<<"Error"<<COURSES_OFFERED_PATH<<std::endl;
    return;
  }
  file<<"Title,Number of Units,Quarter\n"; // encabezado
  std::vector<Course> remainig_courses; 
  ...
  file.close();
  all_courses = remainig_courses; // cursos ofecidos
...
void write_courses_not_offered(std::vector<Course> &unlisted_courses) {
  ...
  if (!file.is_open())
  {
    std::cerr<<"Error"<<COURSES_NOT_OFFERED_PATH<<"\n";
    return;
  }
  file<<"Title,Number of Units,Quarter\n"; 
